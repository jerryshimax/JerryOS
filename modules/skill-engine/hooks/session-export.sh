#!/usr/bin/env bash
# Session Export Hook — exports Claude Code session transcripts to JSONL,
# ingests them into the Memory MCP session database, and refreshes handoff
# snapshots for downstream bootstrap tooling.
#
# Called by SessionEnd hook in settings.json.
# Reads from ~/.claude/projects/<project>/<session-id>.jsonl and writes to
# ~/Ship/logs/sessions/ plus ~/Ship/logs/handoffs/.

set -euo pipefail

source "$HOME/Ship/tools/handoff-lib.sh"

SESSIONS_DIR="$HOME/Ship/logs/sessions"
HANDOFF_ROOT="$HOME/Ship/logs/handoffs"
INGEST_CLI="$HOME/Ship/mcp-memory/dist/ingest-cli.js"

mkdir -p "$SESSIONS_DIR" "$HANDOFF_ROOT"
...
if [[ -z "$LATEST_TRANSCRIPT" ]]; then
    # Fall back to the most recent root-level transcript file under Claude's project tree.
    LATEST_MTIME=0
    while IFS= read -r -d '' transcript; do
...
# Ingest into Memory MCP session database.
if [ -f "$OUTPUT_FILE" ] && [ -f "$INGEST_CLI" ]; then
